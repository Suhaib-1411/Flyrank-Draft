...
**Known Limitations**
API Dependency: The agent relies entirely on the uptime and rate limits of the public MITRE CVE API. If the endpoint is down, live extraction halts.
Input Validation: Currently, the script does not perform local Regex validation on the CVE string format (e.g., enforcing CVE-YYYY-NNNN) before dispatching the HTTP request.
